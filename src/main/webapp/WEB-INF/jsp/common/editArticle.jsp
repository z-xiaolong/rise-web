    ...
    <table style="width: 100%;">
        ...
        <tr>
            ...
            <td>
                ...
                <div class="dropdown" style="display: inline;">
                    <button class="btn btn-default dropdown-toggle" type="button" id="menubtn" data-toggle="dropdown"
                        ...
                        <div style="width: 100px; position: absolute">
                            <span id="articleType" >${article.type}</span>
                        </div>

                        <input type="hidden" id = "articleTypeName" value = "${typeID}">
                ...
                <c:choose>
                    <c:when test="${article.subType != null}">
                <div class="dropdown" id="subType" style="display: inline;">
                    </c:when>
                    <c:otherwise>
                        <div class="dropdown" id="subType" style="display: none;"></c:otherwise>
                </c:choose>
                    <button class="btn btn-default dropdown-toggle" type="button" id="subMenubtn" data-toggle="dropdown"
                            style="width: 160px;background-color: #ffffff; text-align: left">
                        <div style="width: 100px; position: absolute">
                            <span id="subArticleType" >${article.subType}</span>
                        </div>

                        <input type="hidden" id = "subArticleTypeName" value = "${subTypeID}">
                        <span class="caret" style="margin-left: 120px"></span>
                    </button>
                    <ul id ="subMenulist" class="dropdown-menu" role="menu" aria-labelledby="subMenubtn" style="margin-top:13px; z-index: 9999">
                        <c:forEach items="${subTypes}" var="item">
                            <li role="presentation" value="${item.id}"><a role="menuitem" href="#" onclick="clickSub(this)">${item.subTypeName}</a></li>
                        </c:forEach>
                    </ul>
                </div>
                <div style="display: inline">
                    <p align="center" style="color: red;padding-left: 30px;width: 150px; display: inline;">发布日期：</p>
                    <input type="text" style="width:242px; height:34px;text-align:left;border: 1px solid #ccc; display: inline;" id="publishDate" value="${article.publishTime}" placeholder="请选择时间" class="form-control"/>
                </div>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                <div style="padding-top: 30px;margin-left: -85px;">
                    <script type="text/plain" id="myEditor" name="myEditor">${article.content}</script>
                </div>
            </td>
        </tr>
    </table>
    <div class="modal-footer">
        <button type="button" class="btn btn-default" onclick="cancelEdit();" data-dismiss="modal">取消</button>
...
<script>
    ...
    function editArticle() {
        ...
        type = $("#articleTypeName").val();
        if(type == null || type == ""){
            $("#validate").modal("toggle");
            $("#errorMsg").text("文章类型不能为空！");
            return;
        }
        subType = $("#subArticleTypeName").val();
    ...
    function confirmEdit() {
        $.ajax({
            type: "POST",
            url: "${pageContext.request.contextPath}/articleManage/editArticle",
            async: true,//默认就是true
            dataType: "json",
            data: {
                id: id,
